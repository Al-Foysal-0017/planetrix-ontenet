...
export default function PlanetGroup({ scrollYProgress }) {
  ...
  return (
    <group>
      {PLANETS.map((planet, index) => (
        <group key={planet.id} ref={(el) => (groupRef.current[index] = el)}>
          {/* ৩ডি গোলকের বদলে এখানে Drei-এর Image কম্পোনেন্ট ব্যবহার করা হয়েছে */}
          <Image 
            url={planet.img} 
            transparent 
            scale={[2, 2]} // ইমেজের সাইজ
            toneMapped={false} // এটি অরিজিনাল উজ্জ্বল কালার নিশ্চিত করবে
          />
